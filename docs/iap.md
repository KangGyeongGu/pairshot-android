...
| 티어 | 사용 조건 | 페어 한도 | 광고 |
|---|---|---|---|
| **Trial** | paywall 에서 시작, 14일 (1회만) | 무제한 | 없음 |
| **Free** | trial 종료 후, 또는 trial 받지 않고 진입 | **하루 5개 생성** | 표시 (현행 유지) |
| **Paid** | 구독 활성 | 무제한 | 없음 |

### 핵심 규칙
- "하루 5개 생성" = 디바이스 로컬 자정(00:00 KST) 기준 매일 0으로 리셋. `beforeTimestamp` 가 오늘 00:00 이상인 페어 카운트로 판정
- 누적 보관 개수에는 제한 없음. 어제 만든 페어는 그대로 유지
- 한도 도달 시 paywall (dismissible) 노출
...
| SKU | 가격 | 비고 |
|---|---|---|
| `pairshot_pro` / `monthly` | **₩4,500 / 월** | trial14 offer 부착 |
| `pairshot_pro` / `yearly` | **₩43,200 / 년** | 월 ₩3,600 환산, 약 20% 할인. trial 없이 즉시 결제 |

월·연 둘 다 같은 base subscription 의 별도 base plan 으로 구성. 14일 trial 은 monthly 에만 부여.
...
class CanCreatePairUseCase
    ...
    constructor(
        ...
        suspend operator fun invoke(): Result {
            ...
            val sinceEpochMs = todayStartEpochMs(clock)
        ...
        companion object {
            ...
            fun todayStartEpochMs(clock: Clock): Long =
                LocalDate.now(clock)
...
### 10.1 v1.1.6 보유 페어 — 영향 없음
정책이 *일일 생성 한도* 로 바뀌었으므로 기존 누적 페어 수와 무관. 어제 / 그제 / 작년에 만든 페어는 그대로 사용 가능. 오늘 만든 페어가 5개 이상이면 그 다음 시점부터 paywall 진입 — 다음날 00:00 부터 다시 5개 생성 가능.

→ 기존 v1.1.6 사용자가 업데이트 받자마자 *오늘 처음으로 페어 만들 때* 부터 일일 한도 적용. **그 동안 만든 페어 손해 없음**.
